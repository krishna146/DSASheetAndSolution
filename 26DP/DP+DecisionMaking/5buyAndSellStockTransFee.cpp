...
private:
    int recursive(vector<int> &prices, int idx, int buy)
    {
        ...
        if (buy)
        {
            int buyKro = -prices[idx] - tFee + recursive(prices, idx + 1, 0);
    ...
    int topDown(vector<int> &prices, int idx, int buy, vector<vector<int>> &dp)
    {
        ...
        if (buy)
        {
            int buyKro = -prices[idx] - tFee + topDown(prices, idx + 1, 0, dp);
    ...
    int bottomUp(vector<int> &prices)
    {
        ...
        for (int idx = n - 1; idx >= 0; idx--)
        {
            for (int buy = 1; buy >= 0; buy--)
            {
                ...
                if (buy)
                {
                    int buyKro = -prices[idx] - tFee + dp[idx + 1][0];
    ...
    int spaceOptimization(vector<int> &prices)
    {
        ...
        for (int idx = n - 1; idx >= 0; idx--)
        {
            ...
            for (int buy = 1; buy >= 0; buy--)
            {
                ...
                if (buy)
                {
                    int buyKro = -prices[idx] - tFee + dp[0];
